...
local function moveColorTemperature_handler(driver, device, zb_rx)
  if zb_rx.body.zcl_body.move_mode.value ~= ColorControl.types.CcMoveMode.Stop then
      --emit_pushed_event("button3", device)
	emit_held_event("button3", device)
  end
end
...
--OnOff.ID chi co button 1 moi co, nen duong nhien no la push roi.......

--MoveToLevel thi la mac dinh cuar OEM cho push cua button 2. ......
-- Move.ID thi la mac dinh cua OEm cho held cua button 2, nhung o day cung bi qui ve push het
--Move to
...
--MoveTocolorTemperature mac dinh cua OEM la push cho button 3.
--MoveColorTemperature mac dinh cua OEM la push cho button 3.
      [ColorControl.ID] = {
